...
<style>
    ...
    /* Login Compacto */
    .login-box {max-width: 360px; margin: 100px auto; padding: 30px; background: #161b22; border-radius: 12px; border: 1px solid #30363d; box-shadow: 0 4px 16px rgba(0,0,0,0.3)}
    ...
    .sec-body {background: #0d1117; border: 1px solid #21262d; border-radius: 8px; padding: 10px 14px; margin-bottom: 10px}
...
def get_fornecedores() -> List[dict]:
    ...
    except Exception as e: logger.error(f"Erro fornecedores: {e}"); return []

def create_fornecedor(d: dict) -> bool:
    try: sb.table("pc_fornecedores").insert(d).execute(); st.cache_data.clear(); return True
    except Exception as e: logger.error(f"Erro criar fornecedor: {e}"); return False
...
def update_fornecedor(fid: int, d: dict) -> bool:
    ...
    except Exception as e: logger.error(f"Erro atualizar fornecedor: {e}"); return False
...
def delete_fornecedor(fid: int) -> bool:
    ...
    except Exception as e: logger.error(f"Erro desativar fornecedor: {e}"); return False
...
def get_secoes(loja: str) -> List[dict]:
    ...
    except Exception as e: logger.error(f"Erro seções: {e}"); return []

def create_secao(loja: str, nome: str) -> bool:
    try:
        ss = get_secoes(loja); ordem = (max(s["ordem"] for s in ss) + 1) if ss else 1
        sb.table("pc_secoes").insert({"loja": loja, "nome": nome, "ordem": ordem, "ativa": True}).execute()
        st.cache_data.clear(); return True
    except Exception as e: logger.error(f"Erro criar seção: {e}"); return False
...
def update_secao(sid: int, nome: str) -> bool:
    ...
    except Exception as e: logger.error(f"Erro atualizar seção: {e}"); return False

def delete_secao(sid: int) -> bool:
    try: sb.table("pc_secoes").update({"ativa": False}).eq("id", sid).execute(); st.cache_data.clear(); return True
    except Exception as e: logger.error(f"Erro desativar seção: {e}"); return False
...
def create_item(sid: int, d: dict, user: str) -> Optional[str]:
    ...
    try:
        ...
        return None
    except Exception as e: logger.error(f"Erro criar item: {e}"); return "Erro ao salvar."
...
def update_item(iid: int, d: dict) -> Optional[str]:
    ...
    try:
        ...
        return None
    except Exception as e: logger.error(f"Erro atualizar item: {e}"); return "Erro ao atualizar."

def delete_item(iid: int) -> bool:
    try: sb.table("pc_itens").delete().eq("id", iid).execute(); st.cache_data.clear(); return True
    except Exception as e: logger.error(f"Erro deletar item: {e}"); return False
...
def batch_update_status(ids: List[int], new_status: str) -> bool:
    ...
    try:
        ...
        st.cache_data.clear(); return True
    except Exception as e: logger.error(f"Erro batch: {e}"); return False
...
def get_usuarios() -> List[dict]:
    ...
    except Exception as e: logger.error(f"Erro usuários: {e}"); return []
...
def create_usuario(nome: str, email: str, senha: str, acesso: str) -> Optional[str]:
    ...
    except Exception as e: logger.error(f"Erro criar usuário: {e}"); return "Erro ao criar."

def update_usuario(uid: int, d: dict) -> Optional[str]:
    try:
        sb.table("pc_usuarios").update(d).eq("id", uid).execute(); st.cache_data.clear()
        return None
    except Exception as e: logger.error(f"Erro atualizar usuário: {e}"); return "Erro ao atualizar."
...
with st.sidebar:
    ...
    nav = [("📊 Dashboard", "dashboard"), ("📦 Distribuidora", "distribuidora"), ("🎨 Sublimação", "sublimacao"),
           ("📅 Histórico", "historico"), ("📥 Exportar", "exportar"), ("🏭 Fornecedores", "fornecedores")]
    if u["acesso"] == "admin": nav.append(("⚙️ Admin", "admin"))
...
def pagina_dashboard():
    ...
    metrics2 = [
        ...
        ("📊 Itens Ativos", str(len(df[df["status"].isin(STATUS_AT)])), "#8b949e")
    ]
    for i, (lbl, val, cor) in enumerate(metrics2):
        cols2[i].markdown(f"<div class='kpi-card' style='border-top:3px solid {cor}'><div class='kpi-lbl'>{lbl}</div><div class='kpi-val' style='color:{cor}'>{val}</div></div>", unsafe_allow_html=True)
    ...
    with c1:
        ...
        fig = px.bar(s, x="status", y="qtd", color="status", 
                     color_discrete_map={"Pendente":"#D2991E","Aprovado":"#58A6FF","Comprado":"#A371F7","Entregue":"#3FB950","Cancelado":"#F85149"}, 
    ...
    with c3:
        ...
        if not top.empty:
            fig3 = px.bar(top, x="qtd", y="produto", orientation="h", template="plotly_dark", color_discrete_sequence=["#3FB950"])
            fig3.update_layout(showlegend=False, height=200, margin=dict(t=10,b=10,l=10,r=10), paper_bgcolor="#161b22", plot_bgcolor="#0d1117", font=dict(color="#8b949e", size=10), yaxis=dict(tickfont=dict(size=10)))
            st.plotly_chart(fig3, use_container_width=True)
        else:
            st.markdown("<div class='text-muted' style='padding:40px 0;text-align:center'>Sem dados</div>", unsafe_allow_html=True)

    # Valor por Seção
    st.markdown("<div class='text-muted' style='font-size:12px;margin:10px 0 8px'>VALOR POR SEÇÃO</div>", unsafe_allow_html=True)
    ...
    if not sec.empty:
        fig4 = px.bar(sec, x="total", y="secao", orientation="h", color="loja",
                      ...
                      template="plotly_dark")
        fig4.update_layout(legend=dict(bgcolor="#161b22", orientation="h", yanchor="bottom", y=1.02, font=dict(size=10)), 
                          height=180, margin=dict(t=10,b=10,l=10,r=10), paper_bgcolor="#161b22", plot_bgcolor="#0d1117", 
                          font=dict(color="#8b949e", size=10), yaxis=dict(tickfont=dict(size=10)))
        st.plotly_chart(fig4, use_container_width=True)

def pagina_loja(loja: str):
    info = LOJAS[loja]
    st.markdown(f"<h2 style='font-size:18px;font-weight:700;color:#f0f6fc;margin-bottom:12px'>{info['icone']} {info['nome']}</h2>", unsafe_allow_html=True)
    ...
    if st.session_state.get(f"gs_{loja}"):
        st.markdown("<div class='sec-hdr'><span style='font-weight:600;color:#58A6FF;font-size:13px'>Gerenciar Seções</span></div>", unsafe_allow_html=True)
    ...
    if marcados and not is_op():
        st.markdown(f"<div style='background:#1C2128;border:1px solid #30363d;border-radius:6px;padding:6px 10px;margin-bottom:10px'><b style='color:#58A6FF;font-size:12px'>{len(marcados)} selecionado(s)</b></div>", unsafe_allow_html=True)
        ba = st.columns(5)
        ...
        with st.expander(f"📁 {sec['nome']} ({len(itens_all)} itens){pend_txt}", expanded=True):
            if itens:
                for item in itens:
                    ...
                    cols[1].markdown(f"{img_html} <span style='font-weight:600;font-size:13px'>{item.get('produto','')}</span><br><span class='text-muted' style='font-size:11px'>{meta}</span>", unsafe_allow_html=True)
                    
                    cols[2].markdown(f"<span class='text-muted' style='font-size:11px'>{sec['nome']}</span>", unsafe_allow_html=True)
                    cols[3].markdown(f"<span style='font-size:12px'>{item.get('qtd','')} {item.get('unidade','')}</span>", unsafe_allow_html=True)
                    cols[4].markdown(f"<span class='text-muted' style='font-size:11px'>{fmt_brl(item.get('preco_unit',0))}</span>", unsafe_allow_html=True)
                    cols[5].markdown(f"<b style='color:{info['cor']};font-size:13px'>{fmt_brl(item.get('total',0))}</b>", unsafe_allow_html=True)
                    
                    d = days_until(item.get("dt_necessidade"))
                    urg_html = f"<span class='badge' style='background:rgba(248,81,73,.25);color:#F85149'>Atraso {abs(d)}d</span>" if d and d<0 else ""
    ...
    st.markdown(f"<div class='total-bar'><span class='text-muted' style='font-size:12px'>{info['nome']} — Total:</span> <span style='color:{info['cor']};font-size:16px;font-weight:700;margin-left:6px'>{fmt_brl(total_loja)}</span></div>", unsafe_allow_html=True)

def pagina_historico():
    st.markdown("<h2 style='font-size:20px;font-weight:700;color:#f0f6fc;margin-bottom:12px'>📅 Histórico</h2>", unsafe_allow_html=True)
...
def pagina_exportar():
    st.markdown("<h2 style='font-size:20px;font-weight:700;color:#f0f6fc;margin-bottom:12px'>📥 Exportar</h2>", unsafe_allow_html=True)
    c1,c2 = st.columns(2)
...
def pagina_fornecedores():
    st.markdown("<h2 style='font-size:20px;font-weight:700;color:#f0f6fc;margin-bottom:12px'>🏭 Fornecedores</h2>", unsafe_allow_html=True)
